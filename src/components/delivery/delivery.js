if (/delivery/.test(window.location.href)) {
    var map = $('#delivery-map');
    var tracksImage = $('.delivery-tracks');
    ...
    function setDeliveryHandlers() {
        ...
        function showAmply(element) {
            ...
            sidePanel.handle(link);
            $('html, body').animate({
              scrollTop: $('.slide-panel').offset().top
            }, 400);
        }
        ...
        $('.delivery-logo').on('mouseenter', function() {
            var myIndex = $(this).index('.delivery-logo');

            var el = $('.delivery-column').eq(myIndex);
            el.siblings('.delivery-column').fadeOut();
            el.slideDown();
        });
    }

    function showDeliveryCompany() {
        $('.delivery-column').css('display', 'block');
    }
